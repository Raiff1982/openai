...
public class MyBot
{
    ...
    public MyBot(SentimentAnalysis sentimentAnalysis, ILogger<MyBot> logger)
    {
        _sentimentAnalysis = sentimentAnalysis;
        _logger = logger;
    }

    public async Task GenerateResponse(string message, string userId)
    {
        var prediction = _sentimentAnalysis.Predict(message);
        _logger.LogInformation($"User: {userId}, Message: {message}, Sentiment: {prediction.Prediction}");
